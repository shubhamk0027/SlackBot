...
public class DeletePayload {
    ...
    private static final Logger logger = LoggerFactory.getLogger(DeletePayload.class);
    ...
    public static View buildView() {
        return view(view -> view
                .callbackId("del-payload")
                .type("modal")
                .notifyOnClose(true)
                .title(viewTitle(title -> title.type("plain_text").text("Delete a Payload").emoji(true)))
                ...
                .blocks(asBlocks(
                        input(input -> input
                                ...
                                .element(plainTextInput(pti -> pti.actionId("del-payload")))
                        ...
                        input(input -> input
                                ...
                                .optional(true)
                                .element(plainTextInput(pti -> pti.actionId("del-payload").multiline(true)))
    ...
    public static final ViewSubmissionHandler submissionHandler = (req, ctx) -> {
        logger.info("Verifier running on del payload");
        Map <String, String> errors = new HashMap <>();
        Map<String, Map <String, ViewState.Value>> stateValues = req.getPayload().getView().getState().getValues();

        String teamKey = stateValues.get("teamKey-block").get("del-payload").getValue();
        String method = stateValues.get("method-block").get("del-payload").getValue() ;
        String path = stateValues.get("path-block").get("del-payload").getValue();
        String query = stateValues.get("query-block").get("del-payload").getValue();
        String queryRegex = stateValues.get("queryRegex-block").get("del-payload").getValue();
